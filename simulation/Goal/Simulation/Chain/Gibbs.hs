...
bulkGibbsChain
    :: ( Bilinear f m n, KnownNat k, Hierarchical '[f] '[m,n]
       , ExponentialFamily m, Generative Natural m
       , ExponentialFamily n, Generative Natural n )
      => Natural # Harmonium f m n -- ^ The harmonium
     -> Sample k n -- ^ The initial states of the Gibbs chains
     -> Random s (Chain (Sample k (Harmonium f m n))) -- ^ The resulting Gibbs chains
{-# INLINE bulkGibbsChain #-}
bulkGibbsChain hrm z0s = do
    xzs0 <- upwardPass hrm z0s
    dstp <- accumulateRandomFunction0 (downwardPass hrm)
    ustp <- accumulateRandomFunction0 (upwardPass hrm)
    return . accumulateCircuit0 xzs0 $ proc ((),xzs) -> do
        xzs' <- dstp -< fst $ hUnzip xzs
        ustp -< snd $ hUnzip xzs'
