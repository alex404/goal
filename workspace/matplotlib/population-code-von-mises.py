...
l1, = axes2[0].plot(data["zs"], data["linear-sum-of-tuning-curves"], lw=4, label="Sum of Tuning Curves")
l2, = axes2[0].plot(data["zs"], data["linear-regression"], lw=4, linestyle='--', label="Sinusoid Fit")
l3, = axes[0].plot(data["zs"], data["linear-tuning-curves"][0], lw=4, color='black', label="Tuning Curves")
for curve in data["linear-tuning-curves"][1:]:
    axes[0].plot(data["zs"], curve, lw=4, color='black')

# Plot for affine population
axes2[1].plot(data["zs"], data["affine-sum-of-tuning-curves"], lw=4)
axes2[1].plot(data["zs"], data["affine-regression"], lw=4, linestyle='--')
for curve in data["affine-tuning-curves"]:
    axes[1].plot(data["zs"], curve, lw=4, color='black')
...
plot_file_path = get_plot_path("population-code-von-mises.png")
