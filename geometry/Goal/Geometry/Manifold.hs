...
-}
newtype Point c x = Point {coordinates :: S.Vector (Dimension x) Double}
...
{- | A combination of 'splitReplicated' and 'fmap', where the value of the mapped
function is also a point.
-}
mapReplicatedPoint ::
...
instance
    (Manifold x, Manifold y, Transition c d x, Transition c d y) =>
    Transition c d (x, y)
    where
    {-# INLINE transition #-}
    transition cxy =
