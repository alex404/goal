...
module Goal.Geometry.Differential.Optimization  where
--    (
--    -- * Cauchy Sequences
--      cauchyLimit
--    , cauchySequence
...
cauchyLimit
    :: Ord x
    => (Point c m v x -> Point c m v x -> x) -- ^ Distance (divergence) from previous to next
    -> x -- ^ Epsilon
    -> [Point c m v x] -- ^ Input sequence
    -> Point c m v x
{-# INLINE cauchyLimit #-}
...
cauchySequence
    ...
    => (Point c m v x -> Point c m v x -> x) -- ^ Distance (divergence) from previous to next
    -> x -- ^ Epsilon
    -> [Point c m v x] -- ^ Input list
    -> [Point c m v x] -- ^ Truncated list
...
gradientSequence
    :: (Riemannian c m v x, GVector v x, RealFloat x)
    => x -- ^ Step size
    -> (forall z. RealFloat z => BPoint c m z -> z)  -- ^ Function to minimize
    -> Point c m v x -- ^ The initial point
    -> [Point c m v x] -- ^ The gradient ascent
...
--    -> TangentPair c m v x -- ^ The subsequent TangentPair
--    -> TangentVector c m v x -- ^ The current velocity
--    -> (Point c m v x, TangentVector c m v x) -- ^ The (subsequent point, subsequent velocity)
--{-# INLINE momentumStep #-}
...
--
---- | Momentum ascent.
--momentumSequence :: (Riemannian c m v x, GVector v x, RealFloat x)
--    => x -- ^ Learning rate
--    -> (Int -> x) -- ^ Momentum decay function
--    -> (forall z. RealFloat z => BPoint c m z -> z)  -- ^ Function to minimize
--    -> Point c m v x -- ^ The initial point
--    -> [Point c m v x] -- ^ The gradient ascent with momentum
...
--    -> Int -- ^ Algorithm step
--    -> TangentPair c m v x -- ^ The subsequent gradient
--    -> TangentVector c m v x -- ^ First order velocity
--    -> TangentVector c m v x -- ^ Second order velocity
--    -> (Point c m v x, TangentVector c m v x, TangentVector c m v x) -- ^ Subsequent (point, first velocity, second velocity)
--{-# INLINE adamStep #-}
...
--
---- | Adam ascent.
--adamSequence :: (Riemannian c m v x, GVector v x, RealFloat x)
--    => x -- ^ The learning rate
--    -> x -- ^ The first momentum rate
--    -> x -- ^ The second momentum rate
--    -> x -- ^ Second moment regularizer
...
--    => Vector v k (Point c m v x) -- ^ Independent variable observations
...
--    -> Point c m v x -- ^ Parameter estimates
...
--    :: (Manifold m, GVector v x, RealFloat x)
--    => Point c m v x
--    -> CotangentVector c m v x -- ^ Derivatives
--    -> CotangentTensor c m v x -- ^ Hessian
--    -> Point c m v x -- ^ Step
--{-# INLINE newtonStep #-}
...
--    => (forall z. RealFloat z => BPoint c m z -> z)  -- ^ Function to minimize
--    -> Point c m v x -- ^ Initial point
--    -> [Point c m v x] -- ^ Newton sequence
...
--    -> [CotangentVector c m v x] -- ^ Residual differentials
--    -> Point c m v x -- ^ Parameter estimates
...
--    => x -- ^ Damping Factor
--    -> (Point c m v x -> [x]) -- ^ Residual Function
--    -> (Point c m v x -> [Differentials :#: Tangent c m]) -- ^ Residual Differential
--    -> (Point c m v x) -- ^ Initial guess
--    -> [Point c m v x] -- ^ Gauss-Newton Sequence
