...
testEpoch dgs' ttl (k,ry,hrm1) = do
    ...
    goalWriteFile (mnstlyr1dr ++ "/" ++ ttl) ("ryhrm" ++ show epch) $ show (ry,hrm1)
...
main = do
    ...
    let nyz0 = zero (Tensor ym zm)
        ry0 = ny0
        hrm10 = joinHarmonium ny0 nz0 nyz0
    ...
    let rmly = accumulateMealy0 (0,ry0,hrm10) $ proc (zs,(k,!ry,!hrm1)) -> do
    ...
    let smly = accumulateMealy0 (0,ry0,hrm10) $ proc (zs,(k,!ry,!hrm1)) -> do
            dry <- rctfrmly -< (ry,hrm1)
            dhrm1y <- cdymly -< (ry,hrm1)
    ...
    let (rnlls,snlls) = unzip nllss

    goalWriteFile (mnstlyr1dr ++ "/rectified") "rnlls" $ show rnlls
    goalWriteFile (mnstlyr1dr ++ "/standard") "snlls" $ show snlls
    goalRenderableToSVG mnstlyr1dr "rectified/nlls" 500 250 $ likelihoodRenderable rnlls
    goalRenderableToSVG mnstlyr1dr "standard/nlls" 500 250 $ likelihoodRenderable rnlls
