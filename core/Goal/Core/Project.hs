...
-- | This module provides functions for managing a data-analysis project which
-- applies the Goal libraries as part of its data-processing pipeline. This
-- module also provides functions for applying gnuplot scripts to .csv files
-- output by Goal. These functions use the XDG directory specification to manage
-- files in @$XDG_DATA_HOME\/goal\/project-name$@, where @project-name@ is a
-- 'String' (wrapped up in the 'Project' newtype) which specifies the name of
-- the project.
--
-- The project folder contains (up to) the following 4 folders: @import@,
-- @data@, @export@, @plots@. These folders have the following semantics:
-- @import@ contains .csv files which constitute the raw data to be fed into the
-- project. @data@ contains Haskell .dat files which are parsed by 'Read' and
-- 'Show', and constitute the intermediate data processing IO required by the
-- data-analysis task. @export@ contains .csv that result from the data-analysis
-- and require no further processing by the Goal libraries. Finally, @plots@
-- contains images generated by @goal-plot@, which is a simple program for
-- running gnuplot scripts on the files in @export@.
...
-- | A datatype for representing an 'Experiment', which we interpret as a
-- collection of datasets. An 'Experiment' is always assumed to be part of a
-- larger project.
data Experiment = Experiment
    { projectName :: String
    , experimentName :: String }

-- | A subanalysis within an 'Experiment', for when an 'Experiment' contains
-- multiple datasets, or supports multiple analyses of the data.
