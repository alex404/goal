...
import GHC.TypeNats
import Prelude hiding (concat, concatMap, map, replicate, sum)
...
breakEvery ::
    forall v n k a.
    (G.Vector v a, G.Vector v (Vector v k a), KnownNat n, KnownNat k) =>
    Vector v (n * k) a ->
    Vector v n (Vector v k a)
{-# INLINE breakEvery #-}
breakEvery v0 =
...
range ::
    forall v n x.
    (G.Vector v x, KnownNat n, Fractional x) =>
    x ->
    x ->
    Vector v n x
{-# INLINE range #-}
...
deriving instance
    (KnownNat m, KnownNat n, Numeric x, Num x) =>
    Num (Matrix S.Vector m n x)
deriving instance
    (KnownNat m, KnownNat n, Numeric x, Fractional x) =>
    Fractional (Matrix S.Vector m n x)
deriving instance
    (KnownNat m, KnownNat n, Numeric x, Floating x) =>
...
toRows ::
    (G.Vector v a, G.Vector v (Vector v n a), KnownNat n, KnownNat m) =>
    Matrix v m n a ->
    Vector v m (Vector v n a)
{-# INLINE toRows #-}
...
transpose ::
    forall v m n a.
    (KnownNat m, KnownNat n, G.Vector v Int, G.Vector v a, G.Vector v (Vector v m a)) =>
    Matrix v m n a ->
    Matrix v n m a
{-# INLINE transpose #-}
transpose (Matrix v) =
...
outerProduct ::
    ...
    , G.Vector v Int
    , G.Vector v x
    , G.Vector v (Vector v n x)
    , G.Vector v (Vector v m x)
    , G.Vector v (Vector v 1 x)
    ) =>
    Vector v n x ->
    Vector v m x ->
    Matrix v n m x
...
weakDotProduct v1 v2 = G.foldl foldFun 0 (G.enumFromN 0 (G.length v1) :: S.Vector Int)
  where
    foldFun d i = d + G.unsafeIndex v1 i * G.unsafeIndex v2 i
