...
--class (KnownNat n, 1 <= n) => Determinant (n :: Nat) where
--    determinantV :: Num x => Matrix n n x -> x
--
--instance Determinant 1 where
--    determinantV (Matrix v) = headV v
--
--instance Determinant 2 where
--    determinantV (Matrix (Vector v)) =
--        let [a,b,c,d] = V.toList v
--         in a*d - b*c
--
--instance (KnownNat n, 3 <= n) => Determinant n where
--    determinantV mtx = undefined
--
...
imapV :: (forall (k :: Nat)  . Proxy k -> a -> b) -> Vector n a -> Vector n b
imapV f = undefined
...
--    determinant (Matrix (Vector v)) =
--        let [a,b,c,d] = V.toList v
--         in a*d - b*c

--determinant0 :: (KnownNat n, Num a) => Proxy n -> Matrix n n x -> x
--{-# INLINE determinant0 #-}
--determinant0 prxyn mtx =
--    let n = natValInt prxyn
--     in if n == 1
--           then headV $ toVector mtx
--           else undefined
--
--minorMatrix
--    :: (KnownNat i, KnownNat j, Num x)
--    => Proxy i
--    -> Proxy j
--    -> Matrix n n x
--    -> Matrix (n-1) (n-1) x
--minorMatrix prxi prxj mtx = undefined
--
-- | The dot product of two numerical 'Vector's.
--dotProduct :: Num a => Vector n a -> Vector n a -> a
--{-# INLINE dotProduct #-}
