...
epoch (ncs,alss0) (epch,eps,nstpz) = do
    ...
    let [NC1 ncrcef1',NC1 ncrccd1',NC1 ncrcef3',NC1 ncrccd3'] = ncrcs'
    print . maximum . map abs . listCoordinates $ neuralNetwork ncrcef1'
    print . maximum . map abs . listCoordinates $ neuralNetwork ncrccd1'
    print . maximum . map abs . listCoordinates $ neuralNetwork ncrcef3'
    print . maximum . map abs . listCoordinates $ neuralNetwork ncrccd3'
    ...
    let hstchn = proc () -> do
            x <- xchn' -< ()
            n <- nmly' -< x
            blfs <- mapMealy (ekflt:kflt:hrmflts) -< n
            returnA -< flip squaredError x <$> (dcdn >.>* n):blfs
...
main = do
    ...
    g1ef0 <- runWithSystemRandom $ initialize (Standard # fromList Normal [0,0.000001]) mg1
    g1cd0 <- runWithSystemRandom $ initialize (Standard # fromList Normal [0,0.000001]) mg1
    g3ef0 <- runWithSystemRandom $ initialize (Standard # fromList Normal [0,0.000001]) mg3
    g3cd0 <- runWithSystemRandom $ initialize (Standard # fromList Normal [0,0.000001]) mg3

    let ncrcef1 = HarmoniumCircuit emsn g1ef0 amtx1 bmtx1 dcdy1 dcdz2 y01 Nothing
        ncrccd1 = HarmoniumCircuit emsn g1cd0 amtx1 bmtx1 dcdy1 dcdz2 y01 (Just 1)
        ...
        ncs0 = [NC1 ncrcef1,NC1 ncrccd1,NC1 ncrcef3,NC1 ncrccd3]

    ([NC1 ncrcef1',NC1 ncrccd1',NC1 ncrcef3',NC1 ncrccd3'],alss0)
        <- foldM epoch (ncs0,replicate 4 []). take nepch $ zip3 [0..] epss nstpzs
