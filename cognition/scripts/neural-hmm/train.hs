...
epoch (ncs,alss0) (epch,eps,nstpz) = do
    ...
    x0 <- runWithSystemRandom $ randomElement xcat
    ...
    let p0 = fromList mx [0.33,0.33]
        optflt = parametricFilter (transition . discretePrediction trns . transition) (rectifiedBayesRule emsn) p0

    let hstchn = proc () -> do
            x <- xchn' -< ()
            n <- nmly' -< x
            blfs <- mapMealy (optflt:hrmflts) -< n
